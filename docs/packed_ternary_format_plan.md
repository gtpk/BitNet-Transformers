...
4. RT-112도 통과했다. 우리 tiny per-tensor-native 모델은 ternary-dense Path A'
   (`Wq=gamma*T`)를 통해 x86 I2_S에서 F16/F32 PPL parity를 냈고, latent Path A는
   absmax-vs-absmean control로 붕괴했다.
5. RT-113 / EXPORT-006/007도 통과했다. x86 tiny artifact에서 I2_S는 target
   linears 기준 f32 대비 16x 작고, llama-bench token-generation 기준 약 2x
   빠르다.
6. 다음은 같은 경로를 더 큰 pretrained/small model로 올려 whole-file ratio와
   latency 이득이 유지되는지 확인하는 것이다.
7. TurboQuant KV cache는 weight runtime 경로가 더 큰 모델에서도 닫힌 뒤
   별도 축으로 진행한다.
