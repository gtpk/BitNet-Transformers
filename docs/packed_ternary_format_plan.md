...
### Phase 3: reference matmul

- packed -> unpack -> dense `alpha*T` 로 reference `F.linear`.
- 정확성만 본다(속도 아님). packed 경로와 dense 경로 logit 동일.

### Phase 4: kernel PoC 판단

- CPU에서 unpack-free 또는 fused dequant matmul PoC.
...
format/TC가 고정되고 모델 단위 export가 logit 동일성을 통과하면:

1. reference matmul로 packed 경로 정확성 확정
2. kernel 타겟(우선 CPU, 그 다음 Metal/CUDA/bitnet.cpp) 스코핑
3. TurboQuant KV cache는 weight format이 런타임에서 검증된 뒤 별도 축으로
